Feature: API_US_15 As an administrator, I want to update the status information of existing categories via API connection.
  ...
  @API
  Scenario Outline: api/categories/status/{{id}} endpoint, the returned status code should be 200, and the message in the response body should be verified as "Status changed"
  ...
  Scenario: api/categories/status/{{id}} endpoint, the returned status code should be 203, and the message in the response body should be verified as "No id"
  ...
  Scenario Outline: api/categories/status/{{id}} endpoint, the returned status code should be 203, and the message in the response body should be verified as "No category"
  ...
  Scenario Outline: api/categories/status/{{id}} endpoint, the returned status code should be 401, and the error message in the response body should be verified as "Unauthorized request"
  ...
  Scenario Outline: The update of the desired category status record through the API should be verified. This can be confirmed by sending a GET request to the api/categories/details/{{id}} endpoint with the Status Updated id returned in the response body to ensure that the status record has been successfully updated
